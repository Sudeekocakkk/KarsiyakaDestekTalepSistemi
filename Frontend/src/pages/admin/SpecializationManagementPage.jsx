...
const SpecializationManagementPage = () => {
  ...
  const filteredSpecializations = useMemo(
    () => specializations.filter((spec) => matchesSearch(searchTerm, [spec.name])),
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-xl2 bg-white p-5 shadow-card">
        ...
        {isLoading ? (
          ...
          <ScrollableListContainer rowCount={filteredSpecializations.length}>
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-slate-100 text-xs uppercase tracking-wide text-slate-400">
                  <th className="sticky top-0 z-10 bg-white pb-2 pr-4">Ad</th>
                  <th className="sticky top-0 z-10 bg-white pb-2 pr-4">Açıklama</th>
                  ...
                  <tr key={spec.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50">
                    ...
                    <td className="py-2.5 pr-4">
                      <div className="flex justify-end gap-2">
                        ...
                        <Tooltip content={spec.isActive ? "Pasifleştir" : "Aktifleştir"}>
                          <button
                            type="button"
