...
const DepartmentManagementPage = () => {
  ...
  const filteredDepartments = useMemo(
    () => departments.filter((department) => matchesSearch(searchTerm, [department.name])),
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-xl2 bg-white p-5 shadow-card">
        ...
        {isLoading ? (
          ...
          <ScrollableListContainer rowCount={filteredDepartments.length}>
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-slate-100 text-xs uppercase tracking-wide text-slate-400">
                  <th className="sticky top-0 z-10 bg-white pb-2 pr-4">Müdürlük Adı</th>
                  <th className="sticky top-0 z-10 bg-white pb-2 pr-4">Personel Sayısı</th>
